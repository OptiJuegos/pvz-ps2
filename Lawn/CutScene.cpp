...
// Preloads the reanims of whatever is currently in the seed bank (preset
// banks, or the player's picks once the chooser has committed them). Zombie
// seed packets (iZombie) map to their zombie's resources instead.
void CutScene::PreloadSeedBankResources()
{
	if (mBoard->mSeedBank == nullptr)
		return;

	for (int anIndex = 0; anIndex < mBoard->mSeedBank->mNumPackets; anIndex++)
	{
		SeedPacket& aPacket = mBoard->mSeedBank->mSeedPackets[anIndex];
		if (aPacket.mPacketType == SeedType::SEED_NONE)
			continue;

		if (Challenge::IsZombieSeedType(aPacket.mPacketType))
		{
			Zombie::PreloadZombieResources(Challenge::IZombieSeedTypeToZombieType(aPacket.mPacketType));
			continue;
		}

		Plant::PreloadPlantResources(aPacket.mPacketType);
		if (aPacket.mImitaterType != SeedType::SEED_NONE)
			Plant::PreloadPlantResources(aPacket.mImitaterType);
	}
}
...
#ifdef LOW_MEMORY
	// Preloading every unlocked plant here front-loads dozens of reanim
	// decodes and holds all their definitions at once — fatal on 32 MB. The
	// chooser renders packets from ReanimatorCache frames (built one at a
	// time, definition freed right after in DrawCachedPlant), and the seeds
	// that actually matter are preloaded from the seed bank when the cutscene
	// hands control to the player (see the StartPlaying block in Update).
	PreloadSeedBankResources();
#else
...
#ifdef LOW_MEMORY
	// PurgeSounds() at the board transition dropped every decoded sample, so
	// each SFX would otherwise re-decode on its first mid-gameplay play — a
	// visible hitch per new sound during the level's opening minute. Warm the
	// handful that fires in virtually every level here, where the cost hides
	// inside the level-intro pan.
	if (mApp->mSoundManager != nullptr)
	{
		static const int* const kCommonLevelSounds[] = {
			&Sexy::SOUND_GROAN, &Sexy::SOUND_GROAN2, &Sexy::SOUND_GROAN3,
			&Sexy::SOUND_GROAN4, &Sexy::SOUND_GROAN5, &Sexy::SOUND_GROAN6,
			&Sexy::SOUND_LOWGROAN, &Sexy::SOUND_LOWGROAN2,
			&Sexy::SOUND_SPLAT, &Sexy::SOUND_SPLAT2, &Sexy::SOUND_SPLAT3,
			&Sexy::SOUND_CHOMP, &Sexy::SOUND_CHOMP2, &Sexy::SOUND_CHOMPSOFT,
			&Sexy::SOUND_GULP, &Sexy::SOUND_LIMBS_POP,
			&Sexy::SOUND_THROW, &Sexy::SOUND_THROW2,
			&Sexy::SOUND_PLANT, &Sexy::SOUND_PLANT2, &Sexy::SOUND_SEEDLIFT,
			&Sexy::SOUND_TAP, &Sexy::SOUND_TAP2,
			&Sexy::SOUND_POINTS, &Sexy::SOUND_READYSETPLANT,
			&Sexy::SOUND_AWOOGA, &Sexy::SOUND_FINALWAVE, &Sexy::SOUND_HUGE_WAVE,
			&Sexy::SOUND_SHOVEL, &Sexy::SOUND_PLANTGROW,
		};
		for (size_t i = 0; i < LENGTH(kCommonLevelSounds); i++)
			mApp->mSoundManager->PreloadSound(*kCommonLevelSounds[i]);
	}
#endif
...
#ifdef LOW_MEMORY
		// The chooser freed each plant definition as soon as its packet frame
		// was cached (DrawCachedPlant); reload the ones the player actually
		// took into the level so the first planting doesn't hitch.
		PreloadSeedBankResources();
#endif
		ShowShovel();
		mApp->StartPlaying();
