...
#ifdef LOW_MEMORY
	// The menu's reanimations were just freed above; drop their definitions
	// (selector screen, crazy dave, ...) and the menu's decoded images/sounds
	// before the level loads its own.
	mApp->PurgeUnusedAssets();
	if (mApp->mSoundManager != nullptr && mApp->ShouldPurgeAssets())
		mApp->mSoundManager->PurgeSounds();
#endif
