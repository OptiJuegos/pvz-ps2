...
void PoolEffect::PoolEffectInitialize()
{
    ...
    // Lazy resource image: mBits stays NULL until GetBits() decodes it, and
    // reading through the raw pointer is a fetch from address 0 — garbage on
    // emulators, a TLB exception and silent freeze on real PS2 hardware.
    uint32_t* aCausticBits = aCausticGrayscaleImage->GetBits();
    if (aCausticBits == NULL)
    {
        memset(mCausticGrayscaleImage, 0, 256 * 256); // flat water, no caustics
        return;
    }
    int index = 0;
    for (int x = 0; x < 256; x++)
    {
        for (int y = 0; y < 256; y++)
        {
            mCausticGrayscaleImage[index] = (unsigned char)aCausticBits[index];
            index++;
        }
    }
}
