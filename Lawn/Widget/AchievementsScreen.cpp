...
#ifdef PS2_PLATFORM
	static int sAchievementsUpdateLogs = 0;
	bool aLogAchievementsUpdate = sAchievementsUpdateLogs < 16;
	if (aLogAchievementsUpdate)
	{
		sAchievementsUpdateLogs++;
		TodHesitationTrace("ACHUPDATE begin ptr=%p y=%d scroll=%d visible=%d",
			this, mY, mScrollValue, mVisible ? 1 : 0);
	}
#endif
	if (mScrollValue <= 0)
	{
#ifdef PS2_PLATFORM
		if (aLogAchievementsUpdate)
			TodHesitationTrace("ACHUPDATE end early ptr=%p", this);
#endif
		return;
	}
...
#ifdef PS2_PLATFORM
	if (aLogAchievementsUpdate)
		TodHesitationTrace("ACHUPDATE end ptr=%p y=%d scroll=%d", this, mY, mScrollValue);
#endif
}
...
void AchievementsWidget::Draw(Graphics* g) {
	int aVisibleTop = -mY;
	int aVisibleBottom = aVisibleTop + mApp->mHeight;

	if (0 < aVisibleBottom && IMAGE_SELECTORSCREEN_ACHIEVEMENTS_BG->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_SELECTORSCREEN_ACHIEVEMENTS_BG, 0, 0);

	int aHeight = IMAGE_SELECTORSCREEN_ACHIEVEMENTS_BG->mHeight;
	for (int i = 1; i <= 70; i++)
	{
		int aTileY = aHeight * i;
		if (aTileY < aVisibleBottom && aTileY + IMAGE_ACHEESEMENTS_HOLE_TILE->mHeight > aVisibleTop)
			g->DrawImage(IMAGE_ACHEESEMENTS_HOLE_TILE, 0, aTileY);
	}

	if (1125 < aVisibleBottom && 1125 + IMAGE_ACHEESEMENTS_BOOKWORM->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_BOOKWORM, 0, 1125);
	if (2250 < aVisibleBottom && 2250 + IMAGE_ACHEESEMENTS_BEJEWELED->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_BEJEWELED, 0, 2250);
	if (4500 < aVisibleBottom && 4500 + IMAGE_ACHEESEMENTS_CHUZZLE->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_CHUZZLE, 0, 4500);
	if (6750 < aVisibleBottom && 6750 + IMAGE_ACHEESEMENTS_PEGGLE->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_PEGGLE, 0, 6750);
	if (9000 < aVisibleBottom && 9000 + IMAGE_ACHEESEMENTS_PIPE->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_PIPE, 0, 9000);
	if (11250 < aVisibleBottom && 11250 + IMAGE_ACHEESEMENTS_ZUMA->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_ZUMA, 0, 11250);

	int aChinaY = mHeight - IMAGE_ACHEESEMENTS_CHINA->mHeight - 650;
	if (aChinaY < aVisibleBottom && aChinaY + IMAGE_ACHEESEMENTS_CHINA->mHeight > aVisibleTop)
		g->DrawImage(IMAGE_ACHEESEMENTS_CHINA, 0, aChinaY);
