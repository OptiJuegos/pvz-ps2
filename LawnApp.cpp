...
#ifdef PS2_PLATFORM
#include <malloc.h>                        // mallinfo() (heap-pressure checks)
#include "platform/ps2/Ps2PvzServices.h"  // Ps2PvzServices helpers
#include "platform/ps2/Ps2Trace.h"        // Ps2Trace / Ps2TraceBeat (startup tracer)
#endif
...
LawnApp::LawnApp()
{
	// SexyAppBase created a plain ResourceManager, but TodLib casts
	// mResourceManager to TodResourceManager everywhere (and that class now
	// has data members and a virtual override), so the instance must really
	// be one. Nothing has been registered in it yet at this point.
...
#ifdef LOW_MEMORY
	// DisposeBoard's EffectSystemFreeAll just emptied the reanimation holder,
	// so every level-only definition is now unreferenced and can be dropped.
	PurgeUnusedAssets();
	// PCM is only purged on board transitions: purging at dialog close would
	// cut UI sounds that are still playing.
	if (mSoundManager != nullptr && ShouldPurgeAssets())
		mSoundManager->PurgeSounds();
#endif
}
...
// Purging everything at every transition forced a mass re-decode of images
// and sounds right after (visible as a burst of hitches). The heap ceiling is
// ~26MB (32MB minus the ELF image and stacks), so while actual usage is low
// it is cheaper to keep decoded assets resident and skip the purge entirely.
bool LawnApp::ShouldPurgeAssets()
{
#ifdef PS2_PLATFORM
	struct mallinfo mi = mallinfo();
	return (long)mi.uordblks > (18L << 20);
#else
	return true;
#endif
}

// Frees every asset that nothing alive is using: reanim definitions without a
// live Reanimation (plus their orphaned source images), the pre-rendered
// ReanimatorCache frames, and the decoded pixels of lazy images. Everything
// reloads transparently on next use, so this is safe at any screen
// transition where a heavy screen (board, almanac, store) was just torn down.
void LawnApp::PurgeUnusedAssets()
{
	if (!ShouldPurgeAssets())
	{
		TodTrace("purge skipped: heap under pressure threshold");
		return;
	}
	if (mReanimatorCache != nullptr)
		mReanimatorCache->ReanimatorCachePurge();
	ReanimatorFreeUnusedDefinitions();
	PurgeLazyImages();
}
#endif
...
{
#ifdef PS2_PLATFORM
	TodHesitationTrace("showselector begin");
#endif
	KillBoard();
	//UpdateRegisterInfo();
	if (mGameSelector)
...
#ifdef PS2_PLATFORM
	TodHesitationTrace("showselector addwidget");
#endif
	mWidgetManager->AddWidget(mGameSelector);
#ifdef PS2_PLATFORM
	TodHesitationTrace("showselector bringtoback");
#endif
	mWidgetManager->BringToBack(mGameSelector);
#ifdef PS2_PLATFORM
	TodHesitationTrace("showselector setfocus");
#endif
	mWidgetManager->SetFocus(mGameSelector);
#ifdef PS2_PLATFORM
	TodHesitationTrace("showselector ready");
#endif
...
#ifdef LOW_MEMORY
		// Same as KillAlmanacDialog: reclaim the store's cached frames and
		// definitions, but never mid-level (see the note there).
		if (mBoard == nullptr)
			PurgeUnusedAssets();
#endif
	}
...
bool LawnApp::KillAlmanacDialog()
{
	if (GetDialog(Dialogs::DIALOG_ALMANAC))
	{
		...
		ClearUpdateBacklog(false);
#ifdef LOW_MEMORY
		// The almanac walks every unlocked plant/zombie building cached frames
		// and loading reanim definitions; reclaim them now. Only from the menu:
		// with a board up (almanac via seed chooser) this would drop the
		// preloaded wave-zombie definitions and cause mid-level reload hitches.
		if (mBoard == nullptr)
			PurgeUnusedAssets();
#endif
		return true;
...
#ifdef PS2_PLATFORM
	Ps2Trace(PS2_TRACE_INIT_RESXML);
#endif
...
#ifdef PS2_PLATFORM
	// PS2 has no working on-screen text entry, so the "Enter Your Name" new-user
	// dialog can never be completed and the Game Selector gets stuck in
	// SELECTOR_NEW_USER (buttons never appear). If no profile exists yet,
	// auto-create a default one so the selector goes straight to SELECTOR_IDLE.
	if (mPlayerInfo == nullptr)
	{
		mPlayerInfo = mProfileMgr->AddProfile(__S("Player"));
		if (mPlayerInfo != nullptr)
			mProfileMgr->Save();
	}
#endif
...
#ifdef PS2_PLATFORM
#define LAWN_LOAD_HEARTBEAT() Ps2TraceBeat()  // pulse border per resource (Ps2Trace.c)
#else
#define LAWN_LOAD_HEARTBEAT() ((void)0)
#endif
...
void LawnApp::LoadGroup(const char* theGroupName, int theGroupAveMsToLoad)
{
	...
	TodTraceAndLog("[LOAD] begin group '%s'", theGroupName);
	...
	while (!mShutdown && !mCloseRequest && !mLoadingFailed && TodLoadNextResource())
	{
		...
		LAWN_LOAD_HEARTBEAT();  // border flickers while advancing; static = hung
	}

	if (mShutdown || mCloseRequest)
		return;

	TodTraceAndLog("[LOAD] resources read for '%s'", theGroupName);
	if (mResourceManager->HadError() || !ExtractResourcesByName(mResourceManager, theGroupName))
	{
		TodTraceAndLog("[LOAD] FAILED extracting '%s'", theGroupName);
		ShowResourceError();
		mLoadingFailed = true;
	}
	else
	{
		TodTraceAndLog("[LOAD] done group '%s' %d ms", theGroupName, (int)aTimer.GetDuration());
	}
...
//0x4528E0
#ifdef PS2_PLATFORM
// Loading-stage marker: paints the stage's border colour and logs name + heap.
// No-op unless PS2_TRACE_ENABLE=1 (Ps2Trace.h).
#define LAWN_LOAD_STAGE(pt) Ps2Trace(PS2_TRACE_##pt)

// Silences the mixer for as long as it is in scope. Ties the pause to the whole
// LoadingThreadProc body so the mixer issues no audsrv SIF RPCs while the
// loading thread hammers the IOP with file reads — the collision that freezes
// real hardware. RAII so every early return (load failures, shutdown) resumes
// audio.
namespace Sexy { void Ps2MixerSetLoadPause(bool thePaused); }
struct Ps2MixerLoadPauseGuard
{
	Ps2MixerLoadPauseGuard()  { Sexy::Ps2MixerSetLoadPause(true); }
	~Ps2MixerLoadPauseGuard() { Sexy::Ps2MixerSetLoadPause(false); }
};
#define LAWN_MIXER_PAUSE_DURING_LOAD() Ps2MixerLoadPauseGuard aMixerLoadPauseGuard
#else
#define LAWN_LOAD_STAGE(pt) ((void)0)
#define LAWN_MIXER_PAUSE_DURING_LOAD() ((void)0)
#endif

void LawnApp::LoadingThreadProc()
{
	LAWN_MIXER_PAUSE_DURING_LOAD();  // no audsrv SIF RPCs while we own the IOP
	TodTraceAndLog("[LOAD] loading thread entered");
	LAWN_LOAD_STAGE(LOAD_LOADERBAR);
	if (!TodLoadResources("LoaderBar"))
	{
		TodTraceAndLog("[LOAD] FAILED TodLoadResources('LoaderBar')");
		return;
	}
	TodTraceAndLog("[LOAD] LoaderBar ready");

	TodTraceAndLog("[LOAD] begin LawnStrings");
	LAWN_LOAD_STAGE(LOAD_STRINGS);
	TodStringListLoad("Properties/LawnStrings.txt");
	TodTraceAndLog("[LOAD] LawnStrings ready");

	if (mTitleScreen)
	{
		mTitleScreen->mLoaderScreenIsLoaded = true;
	}
	TodTraceAndLog("[LOAD] title loader screen marked ready");

	LAWN_LOAD_STAGE(LOAD_COUNTTASKS);
	...
	TodTraceAndLog("[LOAD] task count ready: completed=%d total=%d",
		mCompletedLoadingThreadTasks, mNumLoadingThreadTasks);
	...
	LAWN_LOAD_STAGE(LOAD_IMAGES);
	LoadGroup("LoadingImages", 9);
	LAWN_LOAD_STAGE(LOAD_FONTS);
	LoadGroup("LoadingFonts", 54);
	if (mLoadingFailed || mShutdown || mCloseRequest)
	{
		TodTraceAndLog("[LOAD] stopped after image/font resources failed=%d shutdown=%d close=%d",
			(int)mLoadingFailed, (int)mShutdown, (int)mCloseRequest);
		return;
	}
	...
	TodTraceAndLog("[LOAD] begin MusicInit");
	LAWN_LOAD_STAGE(LOAD_MUSICINIT);
	mMusic->MusicInit();
	TodTraceAndLog("[LOAD] MusicInit ready");
	// aDuration goes unused
	//int aDuration = max(aTimer.GetDuration(), 0.0);
	aTimer.Start();

	TodTraceAndLog("[LOAD] begin systems init");
	LAWN_LOAD_STAGE(LOAD_SYSTEMS);
	mPoolEffect = new PoolEffect();
	mPoolEffect->PoolEffectInitialize();
	mZenGarden = new ZenGarden();
	mReanimatorCache = new ReanimatorCache();
	LAWN_LOAD_STAGE(LOAD_REANIMCACHE);
	mReanimatorCache->ReanimatorCacheInitialize();
	TodFoleyInitialize(gLawnFoleyParamArray, LENGTH(gLawnFoleyParamArray));
	TodTraceAndLog("[LOAD] systems init ready");

	TodTrace("loading '%s' %d ms", "stuff", (int)aTimer.GetDuration());
	aTimer.Start();

	TodTraceAndLog("[LOAD] begin trails");
	LAWN_LOAD_STAGE(LOAD_TRAILS);
	TrailLoadDefinitions(gLawnTrailArray, LENGTH(gLawnTrailArray));
	TodTraceAndLog("[LOAD] trails ready");
	TodTrace("loading '%s' %d ms", "trail", (int)aTimer.GetDuration());
	aTimer.Start();
	TodHesitationTrace("trail");
	
	TodTraceAndLog("[LOAD] begin particles");
	LAWN_LOAD_STAGE(LOAD_PARTICLES);
	TodParticleLoadDefinitions(gLawnParticleArray, LENGTH(gLawnParticleArray));
	TodTraceAndLog("[LOAD] particles ready");
	//aDuration = max(aTimer.GetDuration(), 0.0);
	aTimer.Start();

	// The loading thread accumulates decoded title art plus every compiled
	// definition; near the ~26MB heap ceiling the next decode OOMs and hangs
	// the console. Shed decoded bits (textures stay alive) when close.
	TodTraceAndLog("[LOAD] purge before preload");
	LAWN_LOAD_STAGE(LOAD_PURGE);
	PurgeLazyImageBits();
	TodTraceAndLog("[LOAD] purge before preload ready");

	TodTraceAndLog("[LOAD] begin PreloadForUser");
	LAWN_LOAD_STAGE(LOAD_PRELOAD);
	PreloadForUser();
	if (mLoadingFailed || mShutdown || mCloseRequest)
	{
		TodTraceAndLog("[LOAD] stopped after PreloadForUser failed=%d shutdown=%d close=%d",
			(int)mLoadingFailed, (int)mShutdown, (int)mCloseRequest);
		return;
	}
	TodTraceAndLog("[LOAD] PreloadForUser ready");

	//aDuration = max(aTimer.GetDuration(), 0.0);
	aTimer.Start();

	TodTraceAndLog("[LOAD] recount preload tasks");
	GetNumPreloadingTasks();
	TodTraceAndLog("[LOAD] begin LoadingSounds");
	LAWN_LOAD_STAGE(LOAD_SOUNDS);
	LoadGroup("LoadingSounds", 54);
	TodTraceAndLog("[LOAD] LoadingSounds ready");
	TodTraceAndLog("[LOAD] final purge");
	PurgeLazyImageBits();
	LAWN_LOAD_STAGE(LOAD_DONE);
	TodTraceAndLog("[LOAD] loading thread finished");
