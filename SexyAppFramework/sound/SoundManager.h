...
public:
	...
	// Frees decoded sample memory while keeping sounds reloadable on demand.
	// Used by LOW_MEMORY targets at screen transitions; default is a no-op.
	virtual void			PurgeSounds() {}

	// Decodes a sample now so its first play doesn't hitch mid-gameplay.
	// Only meaningful on targets that decode lazily; default is a no-op.
	virtual void			PreloadSound(unsigned int theSfxID) { (void)theSfxID; }
};
