	...
	mApp(theMemoryImage.mApp)
{
	...
	// The copy owns its own bits and never re-decodes from file, so it must
	// not be purged (its source may be gone or the copy may diverge).
	mLazyUnloaded = false;
	mLazyPinned = true;
	mLazyCleanCount = 0;
	mLazyRestoreD3DFlags = 0;
	bool aSourceWasLazy = theMemoryImage.mLazyUnloaded;

	MemoryImage* aNonConstMemoryImage = (MemoryImage*) &theMemoryImage;

	if ((theMemoryImage.mBits == NULL) && (theMemoryImage.mColorTable == NULL)
#ifdef PS2_PLATFORM
		&& (theMemoryImage.mRGBBits == NULL)
#endif
		)
	{
...
#ifdef PS2_PLATFORM
	if (theMemoryImage.mRGBBits != NULL)
	{
		mRGBBits = new uchar[mWidth*mHeight*3];
		memcpy(mRGBBits, theMemoryImage.mRGBBits, mWidth*mHeight*3);
	}
	else
		mRGBBits = NULL;
#endif
	...
	if (deleteBits)
	{
		// Remove the temporary source bits; a lazy source goes back to the
		// not-yet-decoded state so its next GetBits() re-decodes from file.
		delete [] aNonConstMemoryImage->mBits;
		aNonConstMemoryImage->mBits = NULL;
		aNonConstMemoryImage->mLazyUnloaded = aSourceWasLazy;
	}
...
#ifdef PS2_PLATFORM
void MemoryImage::SetCompactBits(uint32_t* theColorTable, uchar* theColorIndices, uchar* theRGBBits, int theWidth, int theHeight, bool hasTrans, bool hasAlpha, bool commitBits)
{
	delete [] mBits;
	mBits = NULL;
	delete [] mColorTable;
	...
	if (theColorTable != NULL && theColorIndices != NULL)
	{
		mColorTable = new uint32_t[256];
		mColorIndices = new uchar[mWidth * mHeight];
		if (mColorTable != NULL && mColorIndices != NULL)
		{
			memcpy(mColorTable, theColorTable, 256 * sizeof(uint32_t));
			memcpy(mColorIndices, theColorIndices, mWidth * mHeight);
		}
		else
		{
			delete [] mColorTable;
	...
	else if (theRGBBits != NULL)
	{
		mRGBBits = new uchar[mWidth * mHeight * 3];
		if (mRGBBits != NULL)
			memcpy(mRGBBits, theRGBBits, mWidth * mHeight * 3);
	}
...
void MemoryImage::AdoptCompactBits(uint32_t* theColorTable, uchar* theColorIndices, uchar* theRGBBits, int theWidth, int theHeight, bool hasTrans, bool hasAlpha, bool commitBits)
{
	delete [] mBits;
	mBits = NULL;
	delete [] mColorTable;
	...
	if (theColorTable != NULL && theColorIndices != NULL)
	{
		mColorTable = theColorTable;
		mColorIndices = theColorIndices;
	}
	else if (theRGBBits != NULL)
	{
		mRGBBits = theRGBBits;
...
uint32_t* MemoryImage::GetBits()
{
	...
	if (mBits == NULL && mLazyUnloaded)
	{
		if (mApp->LoadLazyImageBits(this) && mBits != NULL)
			return mBits;
		// Decode failed: fall through to the zero-filled buffer below so
		// callers still get a valid (blank) surface.
		mLazyUnloaded = false;
	}
#endif
