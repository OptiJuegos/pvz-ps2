...
static bool Ps2IsGameSelectorScene()
{
	return gGetCurrentLevelName != NULL && gGetCurrentLevelName() == __S("Game Selector");
}
#endif
...
void WidgetContainer::UpdateAll(ModalFlags* theFlags)
{
...
#ifdef PS2_PLATFORM
	static int sRootGameSelectorFrameLogs = 0;
	bool aLogRootGameSelector = mParent == NULL && Ps2IsGameSelectorScene() && sRootGameSelectorFrameLogs < 6;
	if (aLogRootGameSelector)
	{
		TodHesitationTrace("WCU root begin frame=%d widgets=%d flags=%d", sRootGameSelectorFrameLogs, (int)mWidgets.size(), theFlags->GetFlags());
		sRootGameSelectorFrameLogs++;
	}
	int aPs2ChildIndex = 0;
#endif
	...
	while (mUpdateIterator != mWidgets.end())
	{
		...
		{
			TodHesitationTrace(
				"WCU child begin idx=%d ptr=%p xy=%d,%d size=%dx%d vis=%d dirty=%d",
				aPs2ChildIndex,
				aWidget,
				aWidget->mX,
				aWidget->mY,
...
#ifdef PS2_PLATFORM
		if (aLogRootGameSelector)
			TodHesitationTrace("WCU child end idx=%d ptr=%p", aPs2ChildIndex, aWidget);
		aPs2ChildIndex++;
#endif
...
#ifdef PS2_PLATFORM
	if (aLogRootGameSelector)
		TodHesitationTrace("WCU root end frame=%d", sRootGameSelectorFrameLogs - 1);
#endif
}
