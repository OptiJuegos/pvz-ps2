...
static bool Ps2ShouldLogButtonUpdate()
{
	static int sButtonUpdateLogs = 0;
	if (sButtonUpdateLogs >= 96 || gGetCurrentLevelName == NULL || gGetCurrentLevelName() != __S("Game Selector"))
		return false;
...
{
#ifdef PS2_PLATFORM
	bool aLogButtonUpdate = Ps2ShouldLogButtonUpdate();
	if (aLogButtonUpdate)
		TodHesitationTrace("BTNUPDATE begin id=%d ptr=%p over=%d down=%d disabled=%d alpha=%.2f",
			mId, this, mIsOver ? 1 : 0, mIsDown ? 1 : 0, mDisabled ? 1 : 0, mOverAlpha);
#endif
	Widget::Update();
...
#ifdef PS2_PLATFORM
	if (aLogButtonUpdate)
		TodHesitationTrace("BTNUPDATE end id=%d ptr=%p alpha=%.2f dirty=%d",
			mId, this, mOverAlpha, mDirty ? 1 : 0);
#endif
}
