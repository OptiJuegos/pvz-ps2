...
static bool Ps2ShouldLogWidgetManagerUpdate()
{
	static int sWidgetManagerUpdateLogs = 0;
	if (sWidgetManagerUpdateLogs >= 8 || gGetCurrentLevelName == NULL || gGetCurrentLevelName() != __S("Game Selector"))
		return false;

	sWidgetManagerUpdateLogs++;
	return true;
}
#endif
...
#ifdef PS2_PLATFORM
	bool aLogWidgetManagerUpdate = Ps2ShouldLogWidgetManagerUpdate();
	if (aLogWidgetManagerUpdate)
		TodHesitationTrace("WMUPDATE begin widgets=%d dirty=%d", (int)mWidgets.size(), mDirty ? 1 : 0);
#endif
...
#ifdef PS2_PLATFORM
	if (aLogWidgetManagerUpdate)
		TodHesitationTrace("WMUPDATE end dirty=%d", mDirty ? 1 : 0);
#endif
