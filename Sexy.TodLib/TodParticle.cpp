...
void TodParticleLoadDefinitions(ParticleParams* theParticleParamArray, int theParticleParamArraySize)
{
	...
	// @fix: this array used to be left uninitialised (raw heap garbage) after the
	// new[] above. On platforms where freshly-allocated memory isn't reliably
	// zeroed (e.g. PS2), any slot whose load failed below would keep dangling
	// mEmitterDefs/mEmitterDefCount values and crash the first time it was used.
	// Run every slot through the same constructor used elsewhere for this type so
	// they all start from a known-safe, empty state.
	for (int i = 0; i < theParticleParamArraySize; i++)
		TodParticleDefinitionConstructor(&gParticleDefArray[i]);
	...
	for (int i = 0; i < gParticleParamArraySize; i++)
	{
		...
		if (!TodParticleLoadADef(&gParticleDefArray[i], aParticleParams.mParticleFileName))
		{
			...
			// @fix: a failed/partial load can leave this slot with a mix of valid
			// and partially-allocated fields (e.g. the XML parse died partway
			// through, after some fields were already filled in). Free whatever
			// was allocated and reset to a safe, empty (0 emitters) definition so
			// nothing downstream ever dereferences a stale/garbage pointer for
			// this particle effect -- it will just silently spawn no emitters.
