...
// NOTE: instances must actually be constructed as TodResourceManager (LawnApp
// replaces SexyAppBase's plain ResourceManager in its constructor). The code
// base casts mResourceManager to this type everywhere, so a base instance
// would make the data members below read/write past the allocation and the
// virtual override never dispatch.
class TodResourceManager : public ResourceManager
{
public:
	// Several screens share DelayLoad groups (e.g. DelayLoad_Background3 is
	// held by CutScene, the credits and ChallengeScreen at overlapping times).
	// Each TodLoadResources takes a reference; DeleteResources only really
	// frees the group when the last holder lets go.
	std::map<std::string, int, StringLessNoCase>	mGroupRefCounts;
	...
	int					DeleteUngroupedImages(const std::set<Image*>& theImages);
	...
	using ResourceManager::DeleteResources;
};
