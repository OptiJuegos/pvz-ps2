...
void TodSandImageIfNeeded(Image* theImage)
{
	...
	{
		aImage->GetBits();  // force a lazy decode; FixPixels bails out on null bits
...
void FixPixelsOnAlphaEdgeForBlending(Image* theImage)
{
...
#ifdef PS2_PLATFORM
	printf("[SAND] FixPixelsOnAlphaEdge %dx%d (%d px)...\n",
		theImage->mWidth, theImage->mHeight, theImage->mWidth * theImage->mHeight);
#endif
	PerfTimer aTimer;
	aTimer.Start();
	...
	// Sanding is deterministic post-processing of the decoded file, so it
	// keeps a lazy image purgeable; re-arm the sanding flag so a purged image
	// gets sanded again after its next decode.
	aImage->mLazyCleanCount = aImage->mBitsChangedCount;
	aImage->mLazyRestoreD3DFlags |= D3DIMAGEFLAG_SANDING;
...
// Refcounted group deletion: a group loaded by N screens through
// TodLoadResources is only truly deleted when the Nth holder releases it.
// Groups loaded outside TodLoadResources (StartLoadResources during boot) have
// no refcount entry and delete immediately, matching the old behavior. The
// empty group ("delete everything", shutdown path) also bypasses refcounts.
void TodResourceManager::DeleteResources(const std::string& theGroup)
{
	if (theGroup.empty())
	{
		mGroupRefCounts.clear();
		ResourceManager::DeleteResources(theGroup);
		return;
	}

	std::map<std::string, int, StringLessNoCase>::iterator anItr = mGroupRefCounts.find(theGroup);
	if (anItr != mGroupRefCounts.end())
	{
		if (--anItr->second > 0)
			return;  // another screen still holds this group
		mGroupRefCounts.erase(anItr);
	}
	ResourceManager::DeleteResources(theGroup);
}
...
// Deletes image entries that match the given pointers. Restricted to entries
// with no resource group: those are the ones AddImageToMap registered for
// lazily-loaded definition images, which have no XML group to reload them and
// no extracted IMAGE_* global pointing at them. Group-owned images keep the
// existing DeleteResources(group) lifecycle. The map entry is fully erased so
// DefinitionLoadImage can re-register it on the next load.
int TodResourceManager::DeleteUngroupedImages(const std::set<Image*>& theImages)
{
	int aCount = 0;
	for (ResMap::iterator anItr = mImageMap.begin(); anItr != mImageMap.end();)
	{
		ImageRes* aRes = (ImageRes*)anItr->second;
		if (aRes->mResGroup.empty() && theImages.find((Image*)aRes->mImage) != theImages.end())
		{
			aRes->DeleteResource();
			delete aRes;
			anItr = mImageMap.erase(anItr);
			aCount++;
		}
		else
			++anItr;
...
#ifdef PS2_PLATFORM
		// Name each resource BEFORE loading it: log.txt lines hit the disk
		// immediately (TodDebug.cpp), so after a freeze the last line of the
		// log is the exact item that wedged the console.
		printf("[RES] %d '%s'\n", (int)aRes->mType, aRes->mPath.c_str());
#endif
		if (!LoadNextResource())
			break;
