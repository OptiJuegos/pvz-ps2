...
{
#ifdef PS2_PLATFORM
    // A throwing new here is a silent console freeze. No purge/retry: this
    // can run while holding Ps2IoLock and PurgeLazyImageBits takes the GL
    // crit sect first (lock-order inversion). Just make the OOM visible.
    void* aPtr = operator new[](theSize, std::nothrow);
    if (aPtr == NULL)
        printf("[DEF] OOM allocating %d bytes\n", theSize);
#else
...
bool DefinitionReadCompiledFile(const SexyString& theCompiledFilePath, DefMap* theDefMap, void* theDefinition)
{
    ...
    aTimer.Start();
#ifdef PS2_PLATFORM
    // Raw stdio hits SIF RPC directly; serialize against the pad-poll thread.
    Ps2IoLockAcquire();
#endif
    // Through the pak layer, not raw stdio: compiled defs ship inside main.pak,
    // which raw fopen can't see (it only finds loose files).
...
bool DefinitionIsCompiled(const SexyString& theXMLFilePath)
{
...
#ifdef PS2_PLATFORM
    Ps2IoLockAcquire();
    int aCompiledStat = stat(aCompiledFilePath.c_str(), &attr);
    Ps2IoLockRelease();
    if (aCompiledStat != 0)
        return false;
    // PS2 runtime builds must not recompile XML definitions on the console:
    // the transient parser/image allocations can leave only a few KB free
    // before the first draw. Treat any shipped loose/host compiled file as
    // authoritative and regenerate it offline when assets change.
    return true;
#else
...
#ifdef PS2_PLATFORM
    Ps2IoLockAcquire();
    int aXMLStat = stat(theXMLFilePath.c_str(), &attr);
    Ps2IoLockRelease();
    if (aXMLStat != 0)
#else
    ...
    {
#ifdef PS2_PLATFORM
        // On PS2 the source XML normally ships inside the pak, so stat() on the
        // loose filesystem fails. The pak is immutable between runs, so a
        // compiled file already present on disk is valid — reuse it instead of
        // recompiling every boot (each reanim compile is several seconds of PNG
        // decoding on the EE). Only fall through to "can't compile" if the
        // source isn't in the pak either.
...
#ifdef PS2_PLATFORM
    // MkDir + raw stdio write hit SIF RPC; serialize against the pad-poll thread.
    Ps2IoLockAcquire();
#endif
    MkDir(aFilePath);
