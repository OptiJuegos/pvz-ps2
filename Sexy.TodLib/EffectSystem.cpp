...
void TodTriangleGroup::AddTriangle(Graphics* g, Image* theImage, const SexyMatrix3& theMatrix, const Rect& theClipRect, const Color& theColor, int theDrawMode, const Rect& theSrcRect)
{
	...
	// @fix: TOD_ASSERT is compiled out entirely in release builds (see
	// TodDebug.h), so a null image (e.g. a particle effect whose texture
	// failed to load) used to fall straight through to the mImage->mWidth /
	// mImage->mHeight dereferences below and crash. Bail out safely instead --
	// this just means that one effect silently doesn't draw.
	if (theImage == nullptr)
		return;
